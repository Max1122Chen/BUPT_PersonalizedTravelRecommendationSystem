...
| FR-006-3 | 类别名称查询 | 中 | 用户输入类别名称查找某个地点附近的服务设施，并根据实际可达路径距离进行排序。类别名称支持模糊匹配，系统会返回相似度高的结果。 | 系统能够基于内存模糊匹配算法（Trie/相似度）查找附近服务设施 |
...
| FR-013-3 | 模糊查询 | 中 | 输入美食名称、菜系、饭店或窗口名称等进行基于内容的模糊查询，支持部分匹配和同义词。 | 系统能够基于内存模糊匹配算法（Trie/相似度）进行基于内容的模糊查询 |
...
| FR-009-3 | 全文检索 | 中 | 可以按日记内容进行全文检索 | 系统能够基于内存全文索引（倒排/分词/相似度）按日记内容进行全文检索 |
...
| 风险 | FR-009-3 | 全文检索的效率 | 需设计高效的内存全文索引构建与查询算法 |
| 风险 | FR-009-5 | AIGC算法的实现和效果 | 需考虑算法的复杂度和效果 |
| 风险 | FR-013-1 | 美食排序的效率和准确性 | 需设计高效的排序算法，特别是前10名的选择 |
| 风险 | FR-013-3 | 模糊查询的准确性和效率 | 需设计高效的内存模糊查找算法与索引构建策略 |
