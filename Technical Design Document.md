...
> 索引的职责边界：数据库索引主要用于（1）数据持久化场景下的准确定位与粗粒度分页/过滤，
> （2）为预加载（全表/按范围加载）提供必要的读取效率。
> 对于关键字查找、模糊匹配、全文检索、以及关联匹配等“搜索计算”能力，
> 需要由内存索引与算法完成；因此不应在数据库层通过 `LIKE/REGEXP/全文检索函数` 或多表联合查询来实现搜索结果计算。
...
> 模糊查询内存化要求：
> - Trie 的构建数据来源于相关表的全量/必要字段预加载（例如 `foods/scenic_areas/tags` 等），
>   具体字段按业务模块的数据结构确定；
> - Trie 在应用启动时或定时任务/手动触发时完成初始化（确保运行时不依赖数据库 `LIKE`）；
> - 运行时 `search(prefix)` 仅在内存 Trie 上执行，返回候选 ID 后再由排序/打分算法完成最终筛选。
